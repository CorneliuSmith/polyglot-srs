...
describe('HandwritingPanel', () => {
  ...
  it('hides itself until the migration is there', async () => {
    mockProfile.mockResolvedValue({ available: false, adapt: false, habits: [], stats: {}, samples: 0, confirmed: 0 })
    renderPanel()
  ...
  it('shows what is kept, and turning it off asks first then deletes', async () => {
    mockProfile.mockResolvedValue({ available: true, adapt: true, habits: [], stats: {}, samples: 4, confirmed: 1 })
    renderPanel()
  ...
  it('does not ask before turning it on, and resets one language or all', async () => {
    mockProfile.mockResolvedValue({ available: true, adapt: false, habits: [], stats: {}, samples: 0, confirmed: 0 })
    renderPanel()
    fireEvent.click(await screen.findByTestId('hand-toggle'))
    expect(window.confirm).not.toHaveBeenCalled()
    await waitFor(() => expect(setHandAdapt).toHaveBeenCalledWith(true))

    mockProfile.mockResolvedValue({ available: true, adapt: true, habits: [], stats: {}, samples: 2, confirmed: 2 })
    renderPanel()
