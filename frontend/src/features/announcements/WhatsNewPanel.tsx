 ...
 * Entry copy lives in the i18n catalogs (`whatsNew.entries.<id>.*`),
 * resolved here at render time so it follows the site language. */
    ...
    >
      <div className="w-full max-w-md max-h-[85vh] overflow-y-auto rounded-2xl bg-white shadow-xl relative">
        ...
        <div className="px-6 pt-6 pb-5">
          <h2 className="text-xl font-bold text-gray-900 mb-4">{t('whatsNew.title')}</h2>
          <ul className="space-y-5">
            {WHATS_NEW.map((entry) => (
              <li key={entry.id} data-testid="whats-new-entry">
                <div className="flex items-baseline gap-2">
                  <h3 className="font-semibold text-gray-800">
                    {t(`whatsNew.entries.${entry.id}.title`)}
                  </h3>
                ...
                <p className="mt-1 text-sm text-gray-600 leading-relaxed">
                  {t(`whatsNew.entries.${entry.id}.body`)}
                </p>
                {entry.link && (
                  <button
                    type="button"
