...
class TestGrammarStrictForm:
    """Grammar drills test the inflection itself: the right lemma in the
    wrong cell (is where am belongs) must fail the drill as WRONG_FORM,
    while vocabulary keeps the lemma leniency."""
    ...
    def test_lemma_match_fails_grammar_drill(self, nlp):
        # StubNLP lemmatizes going/went -> go
        result, feedback = nlp.check_answer("going", "went", self.GRAMMAR)
        assert result == AnswerResult.WRONG_FORM
        assert "wrong form" in feedback.lower()

    def test_family_match_fails_grammar_drill(self, nlp):
        # runs is in run's family but isn't the expected surface form
        result, feedback = nlp.check_answer("runs", "running", self.GRAMMAR)
        assert result == AnswerResult.WRONG_FORM
        assert "wrong form" in feedback.lower()

    def test_lemma_match_still_passes_vocabulary(self, nlp):
        assert nlp.check_answer("going", "went", {"card_type": "vocabulary"})[0] \
            == AnswerResult.CORRECT_SLOPPY
        assert nlp.check_answer("going", "went")[0] == AnswerResult.CORRECT_SLOPPY

    def test_exact_form_still_passes_grammar_drill(self, nlp):
        assert nlp.check_answer("went", "went", self.GRAMMAR)[0] == AnswerResult.CORRECT
        assert nlp.check_answer("Went", "went", self.GRAMMAR)[0] == AnswerResult.CORRECT
