...
| REV-02 | Fill-in-the-blank drill mode with `{{answer}}` markers in sentences | `drill_sentences.sentence` contains `{{answer}}`; frontend renders with input injected at marker |
...
interface SessionCard {
  id: string
  sentence: string       // contains {{answer}}
...
**What:** Replace `{{answer}}` in the sentence string with a controlled input element.
**When to use:** DrillCard display.

```tsx
// src/features/review/DrillCard.tsx
interface Props {
  sentence: string    // e.g. "Я {{answer}} в магазин."
  value: string
  ...
  return (
    <p dir={dir} className="text-xl leading-loose text-center">
      ...
      <input
        ...
        className="inline-block border-b-2 border-primary min-w-[120px] text-center
                   focus:outline-none bg-transparent mx-2"
        dir={dir}
        autoFocus
      />
      {after}
...
```json
{
  ...
  "feedback": "Correct meaning, but check the exact form. Expected: пошёл"
}
```
**Implementation:** Calls `validate_answer_async(language_code, user_input, correct_answer, card_context)` from `backend.services.nlp`.
...
|--------|----------|-----------|-------------------|-------------|
| REV-01 | Due cards returned sorted by next_review | integration | `pytest backend/tests/test_review_api.py::test_due_cards_sorted -x` | ❌ Wave 0 |
| REV-02 | `{{answer}}` marker renders as input in frontend | unit (component) | `cd frontend && npm run test -- DrillCard` | ❌ Wave 0 |
| REV-03 | Rating buttons call submit with correct answer_result | unit (component) | `cd frontend && npm run test -- RatingButtons` | ❌ Wave 0 |
| REV-04 | WRONG_FORM feedback shown after NLP validate | unit (component) | `cd frontend && npm run test -- FeedbackPanel` | ❌ Wave 0 |
| REV-05 | CORRECT_SLOPPY warning shown | unit (component) | `cd frontend && npm run test -- FeedbackPanel` | ❌ Wave 0 |
| REV-06 | Session summary shows correct accuracy % | unit (hook) | `cd frontend && npm run test -- useReviewSession` | ❌ Wave 0 |
| REV-07 | Learn endpoint adds batch_size new cards | integration | `pytest backend/tests/test_review_api.py::test_learn_session -x` | ❌ Wave 0 |
| REV-08 | Review mode only returns next_review <= now() cards | integration | `pytest backend/tests/test_review_api.py::test_due_only_learned -x` | ❌ Wave 0 |
| UX-01 | Dashboard returns due_count, streak, cefr_progress | integration | `pytest backend/tests/test_dashboard_api.py -x` | ❌ Wave 0 |
| UX-02 | Arabic content gets dir="rtl" and Noto Naskh Arabic | unit (component) | `cd frontend && npm run test -- LanguageWrapper` | ❌ Wave 0 |
| UX-03 | Keyboard renders for ru/ar, not for en | unit (component) | `cd frontend && npm run test -- OnScreenKeyboard` | ❌ Wave 0 |
| UX-08 | Rating buttons min height >= 44px | unit (component) | `cd frontend && npm run test -- RatingButtons` | ❌ Wave 0 |
| PROF-01 | Profile returns/updates batch_size | integration | `pytest backend/tests/test_auth.py::test_profile_batch_size -x` | ❌ Wave 0 (partial — test_auth.py exists) |
| PROF-02 | Active language scopes due cards | integration | `pytest backend/tests/test_review_api.py::test_due_scoped_to_language -x` | ❌ Wave 0 |
...
### Wave 0 Gaps
- [ ] `backend/tests/test_review_api.py` — covers REV-01, REV-07, REV-08, PROF-02
- [ ] `backend/tests/test_dashboard_api.py` — covers UX-01
- [ ] `frontend/src/features/review/__tests__/DrillCard.test.tsx` — covers REV-02
- [ ] `frontend/src/features/review/__tests__/RatingButtons.test.tsx` — covers REV-03, UX-08
- [ ] `frontend/src/features/review/__tests__/FeedbackPanel.test.tsx` — covers REV-04, REV-05
- [ ] `frontend/src/features/review/__tests__/useReviewSession.test.ts` — covers REV-06
- [ ] `frontend/src/features/keyboards/__tests__/OnScreenKeyboard.test.tsx` — covers UX-03
- [ ] `frontend/src/components/__tests__/LanguageWrapper.test.tsx` — covers UX-02
