...
- **Definitions.** The top 200 were read by a reader of Tagalog and
  **49 repaired** through `scripts/apply_gloss_overrides.py`; a second
  reader accepted or corrected every one it saw. Faults found: reaches a synonym 26, wrong sense 14, not a definition 4.
...
- `una` was "first (ahead of others)" → "first, ahead of the others, earliest; after the marker sa, it means at first or in the beginning"
- `maliliit` was "plural of maliit" → "small, little - describing more than one thing (plural of maliit)"
