...
The sweep above covered the top 500. Ranks 501-2000 added **35 rows, 16 fatal**, so the
course total is **55 repaired (23 fatal) through rank 2000**.
...
| rank | word | now reads |
| --- | --- | --- |
| 506 | `schau` | look!, have a look (singular imperative of schauen; schau mal — take a l |
| 595 | `drauf` | on it, on that, of it (colloquial form of darauf; es kommt drauf an — it |
| 737 | `tagen` | days (dative plural of Tag; vor drei Tagen — three days ago; in diesen T |
| 848 | `hasse` | I hate (first-person singular present of hassen; ich hasse Kaffee — I ha |
| 982 | `legen` | to lay, to put, to place (causative of liegen; legen Sie die Hände auf d |
| 1066 | `sucht` | he/she/it looks for, searches; you (pl.) look for (third-person singular |
...
- **Definitions.** The top 200 were read by a reader of German and
  **71 repaired** through `scripts/apply_gloss_overrides.py`; a second
  reader accepted or corrected every one it saw. Faults found: a part-of-speech label 40, reaches a synonym 15, wrong sense 12.
  112 of the top 200 now carry a hand-written definition — a low
  count means the extracted glosses were already right, not that the band
  was skipped.
- **Hints.** **28 drill hints** that gave away their answer — sitting
  inside their own translation, or only the agreement feature the drill
  tests — rewritten through `scripts/apply_drill_hints.py`.
- **Headwords.** **6 removed** to `vocab_exclusions.tsv` (letters,
...
- **State.** 98% of the top 1,000 have a sentence the card can
  actually blank; 100% of drills carry an interlinear gloss;
  8 fail-level audit findings; a top-2,000 card is bad — no
  usable sentence, or a fragment drawn — 10% of the time.
...
**176 definitions rewritten** in the rank 201–1000 band. Each stated a
...
- `musst` was "second-person singular present of müssen" → "you must, you have to (second-person singular present of müssen)"
- `gehört` was "past participle of hören" → "heard (past participle of hören); also he/she/it belongs to, is part of (present of gehören)"
