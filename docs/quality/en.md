...
**1,231 English headwords are absent from the course** — nothing can define
them, so the seeder skips them entirely (0 of the 1,231 are in production) —
blank in the file and unresolvable by WordNet — none of them in the top 2,000.
`CHECKS.md` §42.

## The headwords the course never taught (19 Sep 2026)

Nothing can define these, so `seed_english` skips them and the word never
reaches `vocabulary` — verified against production, 0 of 1,231 present. All
1,231 classified maker–checker: **948 are proper nouns** (730 given names,
