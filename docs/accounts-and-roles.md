...
| `ambassador` | grant scope **ignored** | **create accounts, and nothing else.** Signup is invite-only, so each learner is minted by hand; this unblocks whoever is actually recruiting without handing over the rest. Cannot list accounts (that returns every learner's email and study volume), delete them, change plans, grant roles, edit content, or open the review queue. |
...
> **These are siblings, not rungs.** `contributor` (write), `trial_reviewer`
> (check) and `ambassador` (recruit) grant unrelated powers, and an account
> may hold any combination. Only `admin` subsumes all three — `reviewer`
> subsumes the two CONTENT roles and deliberately NOT `ambassador`, because
> approving a sentence has never implied minting logins.

> **Ambassador scope.** The Roles panel still asks for a language when
> granting one, because the column is shared. It is ignored: an account does
> not belong to a language, so "ambassador for Spanish" cannot mean "may
> create Spanish accounts" — there is no such object. A scoped grant confers
> exactly the same power as a global one.
