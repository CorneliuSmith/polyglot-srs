...
| **B — the panel** | endpoint + `ContentHealthPanel` + Deployment "Content" section + targets table; auth on `/api/health`. **Backend landed 18 Sep 2026:** `GET /api/contribute/admin/content-health`, `…/content-health/{code}`, `…/content-health/deploy`, `POST …/content-health/verdicts/{id}`, `GET`/`PUT …/quality-settings`, `PUT …/quality-targets/{code}` (`routers/contribute.py`; status rules in `services/content_health.py`, reads in `repositories/content_health.py`; 69 tests in `test_content_health.py`). GETs degrade to `available: false` before migration 20261107000000, writers 503 naming it. Auth on `/api/health` waits on decision #3, and DEBT "Content Health, phase B" lists what is left out. **Frontend landed 18 Sep 2026:** `ContentHealthPanel` (Admin → Content — worst first, drill-down with plain-SVG sparklines, audit rules, reconcile survey, Agree / Disagree on verdicts), `QualitySettingsPanel` (Admin → Costs — the judge switch, rows per cycle, daily cap, model, per-course targets), Deployment → Content; types and clients in `api/contribute.ts`; tests `ContentHealthPanel.test.tsx`, `QualitySettingsPanel.test.tsx`, `DeploymentPanel.test.tsx`. | none | 3 | the owner can read a trend that was previously impossible |
...
| **E — the judge** | `content_judge` with five questions; gold sets for `sense`, `gloss`, `scripture`, `card_shape` (register's exists); calibration page per question; `content_verdicts`; verdicts routed to existing queues; judge loop behind its flag and cap. **Landed 18 Sep:** `content_judge` (five questions, one schema, three gates); `judge_step.py` in the quality loop behind the admin switch, the daily cap re-checked per batch, the per-course opt-in and `data/eval/calibrated.json` (shipped in the API image by name, pinned by `test_runtime_data_ships.py`); `content_verdicts` written with `run_id`, confidence stored as the four-place Decimal the flagged count compared, retired words out of every scope behind a `column_present` probe; per-pair `quality_runs` rows `judge/tokens` (the spend ledger — `quality_runs`, not `tutor_usage`, because that table's `user_id` is NOT NULL and the judge has no user until decision #2) and `judge/flagged.<question>`; `coverage/judge.<question>` for every course nightly. **Waits on the owner:** migrations 20261029 and 20261030 applied; the switch and cap set in the panel (§9.1); gold sets for the four other questions and the register labels (`calibrated.json` names `register`/`ar` only, provisionally); the service account (§9.2) for J5 routing, which is not built | **yes** (§9.1) | 5 | each question clears the three gates on its gold set before it can write anything but `report` — mechanised: `calibrated.json` is the gate the loop reads |
