...
| **4 — Write: Words and sentences, traced** | **Built 11 Sep 2026** as placement composers: Cyrillic/Latin cursive joins (exit → entry, straight), Arabic positional forms right to left, Hangul blocks by vowel orientation, everything else in cells (`composer.ts`); progressive matching by whole-sequence DTW with a verdict per letter, open-ended for Trace (`matchComposed`); Write → *Trace* over any word or example line, line by line; the same matcher over Free write's ink for a letter-by-letter row; the composed hand above the font in the compare view. **Not yet:** the per-script exceptions (о-joins, hooks, لا, half-forms, headline, marks) and tuning against exemplars — none exist to tune against (DEBT). | 1–2 weeks, ru/ar first |
