...
| **3** | **Phase 8 — example-sentence fitness** | **`xh` wave 1 done 17 Sep 2026: top-1000 452 → 936 of 1000 (45% → 94%), 842 sentences for 484 headwords; the checker removed 21% of what the mechanical gate passed (`docs/quality/xh.md`).** Remaining: `sw` 69%, `yo` 23% (held for tones), `ko` 97%. The authoring queue, worst-first: `yo ko xh th sw` → `tr he tl fa ca id el` → `mi ar it hi ro` → the rest. `pt es de nl fr la ru en ha jam` are already under 15% bad cards and need nothing here. |
